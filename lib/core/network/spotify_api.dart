...
class SpotifyApi {
        ...
        if (error.response?.statusCode == 401 &&
          ...
          if (success) {
            ...
            await isar.writeTxn(() async {
              await isar.authTokens
                  .filter()
                  .serviceEqualTo('spotify')
                  .deleteAll();
            });
            handler.next(error);
          }
  ...
  Future<List<Map<String, dynamic>>> getSavedTracks({int limit = 50}) async {
    final token = await getValidToken();
    if (token == null) {
      throw Exception('Spotify token not found or expired');
    }
  ...
  Future<List<Map<String, dynamic>>> search(String query,
    ...
    try {
      response = await _searchTracksRequest(
        normalizedQuery,
        limit: limit,
        includeMarket: true,
      );
    ...
    final items =
        List<Map<String, dynamic>>.from(response.data['tracks']['items'] ?? []);
  ...
  Future<Response<dynamic>> _searchTracksRequest(
    String query, {
    required int limit,
    ...
    if (includeMarket) {
      queryParameters['market'] = 'from_token';
    }

    return _dio.get('/search', queryParameters: queryParameters);
  }
  ...
  Future<List<Map<String, dynamic>>> getPlaylistTracks(String playlistId,
    ...
    final seenTitleArtist = <String>{};
    String? nextUrl =
        '/playlists/$playlistId/tracks?limit=$limit&market=from_token';
        ...
        for (final item in items) {
          if (item['track'] != null) {
            ...
            final titleArtistKey =
                '${title.toLowerCase()}_${artist.toLowerCase()}';
