...
class SpotifyAuthService {
  ...
  Future<void> authenticate() async {
    try {
      ...
      final callbackUri = Uri.parse(SpotifyApi.redirectUri);
      final callbackScheme = (Platform.isLinux ||
                  Platform.isWindows ||
                  Platform.isMacOS) &&
              (callbackUri.scheme == 'http' || callbackUri.scheme == 'https')
          ? SpotifyApi.redirectUri.replaceFirst('127.0.0.1',
              'localhost') // Replace with localhost for the listener but keep 127 in authorize URL
          : callbackUri.scheme; // Mobile works with just the scheme
      ...
      await isar.writeTxn(() async {
        final existingToken = await isar.authTokens
            .filter()
